...
test('Hockey Smash launches into a full viewport canvas game', async ({ page }) => {
  ...
  await expect(page.locator('#hockey-build-badge')).toContainText('Hockey Smash v0.5.11 · Build 2026-06-29.8');
  ...
  expect(version).toBe('Hockey Smash v0.5.11');
  expect(state.player.health).toBe(100);
  expect(bodyLocked).toBe(true);
  expect(overlayBox?.width).toBeGreaterThan(70);
  expect(overlayBox?.height).toBeGreaterThan(90);
  ...
  const overlayAfterKeyboard = await page.locator('#hockey-player-overlay').boundingBox();
  expect(overlayAfterKeyboard?.x).toBeGreaterThan(overlayBox.x);
  ...
  const beforeGlobalX = await page.evaluate(() => window.RTA_HOCKEY_SMASH.getState().player.x);
  await page.evaluate(() => window.HOCKEY_SMASH_DPAD.press('right'));
  await page.waitForTimeout(120);
  await page.evaluate(() => window.HOCKEY_SMASH_DPAD.release());
  const afterGlobalX = await page.evaluate(() => window.RTA_HOCKEY_SMASH.getState().player.x);
  expect(afterGlobalX).toBeGreaterThan(beforeGlobalX + 20);
...
test('Portrait mobile layout keeps canvas and controls separated', async ({ page }) => {
  ...
  const statusVisible = await page.locator('#hockey-status').isVisible();
  const version = await page.evaluate(() => window.RTA_HOCKEY_SMASH.getVersion());
