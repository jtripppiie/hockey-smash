...
V2 is now the active Hockey Smash path. The root page serves the main v2 game while the project keeps consolidating gameplay into the world-state/canvas-renderer model.
...
One-off and one-at-a-time cast rules belong to world state. For example, Alaska kid cameos are tracked with cast flags so they appear at most once per run and then expire through world lifetimes or projectile dismissal. Mom and Dad also use active entity caps so repeated encounter rolls cannot stack several family cameos at once.
...
- `index.html` serves the main game
