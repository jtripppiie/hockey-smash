...
- `js/games/hockey-smash-encounters.js` owns rain, heavyRain, fastRain, schoolRain fish patterns and combo encounter spawns.
- `js/games/hockey-smash-projectiles.js` owns charged projectiles, stronger arc physics, falling-fish dodge rules, and safe power-ups.
- `js/games/hockey-smash-release.js` is the final release layer so the visible badge/version stays current after older layers load.
...
- `js/games/hockey-smash-input.js`: smooth movement controller for normal play and Computer Mode.
- `js/games/hockey-smash-encounters.js`: moving encounter pass with falling-fish patterns and combo spawns.
- `js/games/hockey-smash-projectiles.js`: charged puck/pointe-shoe projectile layer, salmon dodge rules, and safe power-ups.
- `js/games/hockey-smash-score.js`: distance, score, combo, high score, difficulty, floating text, and run summary layer.
- `js/games/hockey-smash-characters.js`: player customization, Daniel/Sofie mode labels, and character-specific action labels.
- `js/games/hockey-smash-safety.js`: dev unlock, debug logs, countdown, and legacy sideways-salmon guard.
- `js/games/hockey-smash-release.js`: v0.14.4 final release layer.
