...
Bear and eagle art uses clean six-frame transparent sheets. Each frame has measured alpha bounds in `SPRITE_SHEETS`, allowing the renderer to crop unused transparent space without stretching or exposing background artifacts. Moose and salmon deliberately use one clean static image each to avoid sheet-boundary and frame-transition artifacts; their gameplay behavior remains independent from visual frame cycling.
