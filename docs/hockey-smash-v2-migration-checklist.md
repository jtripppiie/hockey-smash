...
`index.html` serves the main v2 game page while the project continues consolidating gameplay into world state and canvas rendering.
...
- salmon score and perfect-catch feedback
- countdown and salmon gate
- current character picker
- current controls
- current scoring
- cameo speed boosts
- Dad joke cameo behavior
- grounded bear walk behavior and moose graze behavior
- player movement acceleration, friction, jump buffer, and coyote time
...
- salmon spawn in the main v2 game only
...
### 11. Add main v2 game splash, mobile layout, fullscreen, and tuning
...
- fullscreen toggle for the main v2 game frame
...
- Alaska kid cameos are non-contact, once per run, say `Hi, you're cute`, time out after about 4-5 seconds, and are projectile-dismissible
...
- Dad and Mom are capped to one active cameo each
...
Manual test the main v2 game:

```text
/
```

Keep improving the main v2 game behavior from `index.html`.
