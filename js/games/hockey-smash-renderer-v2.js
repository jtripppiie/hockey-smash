...
(function () {
  ...
  const SPRITE_SHEETS = Object.freeze({
    bearWalkSheet: {
      ...
      maxWidthScale: 1.54,
      trim: [
        { x: 15, y: 263, w: 338, h: 202 },
        { x: 20, y: 260, w: 317, h: 203 },
        { x: 1, y: 267, w: 331, h: 198 },
        { x: 13, y: 269, w: 330, h: 199 },
        { x: 12, y: 276, w: 324, h: 193 },
    ...
    eagleFlySheet: {
      ...
      maxWidthScale: 1.7,
      trim: [
        { x: 52, y: 154, w: 273, h: 299 },
        { x: 29, y: 204, w: 304, h: 248 },
        { x: 16, y: 260, w: 322, h: 192 },
        { x: 14, y: 354, w: 289, h: 210 },
        { x: 8, y: 186, w: 354, h: 268 },
  ...
  function renderWorld(ctx, world, imageCache = new Map(), options = {}) {
    ...
    // Draw from back to front: background first, then entities, then the player,
    // then effects/debug on top. This is the painter's algorithm.
  ...
  function renderParallaxBackground(ctx, world, imageCache, width, height, options) {
    ...
    // Parallax means far-away layers move slower than close layers.
  ...
  function renderParallaxLayer(ctx, imageCache, layer, width, height, environment) {
    ...
    const offset = Math.round(positiveModulo((environment.scrollX || 0) * speed, layerWidth));

    if (image?.complete && image.naturalWidth) {
      ctx.save();
      ctx.imageSmoothingEnabled = false;
      for (let x = -offset - layerWidth; x < width + layerWidth; x += layerWidth) {
        ctx.drawImage(image, Math.round(x), y, layerWidth, layerHeight);
      }
      ctx.restore();
      return;
    }
    ...
    for (let x = -offset - layerWidth; x < width + layerWidth; x += layerWidth) {
      ctx.fillRect(Math.round(x), y, layerWidth, layerHeight);
    }
    ctx.restore();
  }
  ...
  function renderBubble(ctx, entity, text) {
    ...
    ctx.save();
    ctx.font = '800 12px system-ui, sans-serif';
    const lines = getWrappedLines(ctx, String(text), maxWidth).slice(0, 3);
    const height = Math.max(36, 22 + (lines.length * lineHeight));
    const y = Math.max(18, (entity.y || 0) - height - 10);
    ctx.fillStyle = 'rgba(255, 247, 214, 0.96)';
    ctx.strokeStyle = '#15202c';
    ctx.lineWidth = 3;
    roundRect(ctx, x, y, 188, height, 12);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = '#15202c';
  ...
  function getCycleProgress(environment) {
    const cycleSeconds = Math.max(1, environment.cycleSeconds || 240);
  ...
  function getNightAmount(environment) {
    ...
    if (environment.midnightSun) return 0;
    const progress = getCycleProgress(environment);
    const cycleNight = Math.max(0, Math.min(1, (Math.cos(progress * Math.PI * 2) + 1) / 2));
    return environment.midnightSun ? cycleNight * 0.18 : cycleNight;
  }
